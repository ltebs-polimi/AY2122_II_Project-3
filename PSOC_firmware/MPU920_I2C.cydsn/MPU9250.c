...
void MPU9250_Start(void) {
    ...
    MPU9250_SetSampleRateDivider(4); // From 1kHz (FS) to 200 Hz sampling (4) 
...
//    MPU9250_I2C_Write(MPU9250_I2C_ADDRESS, MPU9250_CONFIG_REG, 0x02);
    // First set up configuration register so that DLPF CFG is set to 0 (to obtain 8kHz)
    ...
    MPU9250_I2C_Write(MPU9250_I2C_ADDRESS, MPU9250_GYRO_CONFIG_REG, temp); //to obtain 32kHz (bypass DPLF)
    
    // Set accelerometer DLPF configuration
    MPU9250_I2C_Write(MPU9250_I2C_ADDRESS, MPU9250_ACCEL_CONFIG_REG, 0x02); //sbagliato voelva mettere 02 in accel_config_reg2 per ottenre 4khz
