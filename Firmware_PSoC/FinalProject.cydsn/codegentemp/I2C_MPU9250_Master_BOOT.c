/*******************************************************************************
* File Name: I2C_MPU9250_Master_BOOT.c
* Version 3.50
*
* Description:
...
static uint8 XDATA I2C_MPU9250_Master_slReadBuf[I2C_MPU9250_Master_BTLDR_SIZEOF_READ_BUFFER];

/* I2C read buffer - The host reads responses from it */
static uint8 XDATA I2C_MPU9250_Master_slWriteBuf[I2C_MPU9250_Master_BTLDR_SIZEOF_WRITE_BUFFER];


/*******************************************************************************
* Function Name: I2C_MPU9250_Master_CyBtldrCommStart
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2C_MPU9250_Master_slWriteBuf - The global variable used to store a received
*                                command.
*  I2C_MPU9250_Master_slReadBuf -  The global variable used to store a response.
...
void I2C_MPU9250_Master_CyBtldrCommStart(void) CYSMALL 
{
    ...
    I2C_MPU9250_Master_SlaveInitWriteBuf(I2C_MPU9250_Master_slWriteBuf, I2C_MPU9250_Master_BTLDR_SIZEOF_WRITE_BUFFER);
    I2C_MPU9250_Master_SlaveInitReadBuf (I2C_MPU9250_Master_slReadBuf, 0u);
...
/*******************************************************************************
* Function Name: I2C_MPU9250_Master_CyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: I2C_MPU9250_Master_CyBtldrCommReset
********************************************************************************
*
* Summary:
...
void I2C_MPU9250_Master_CyBtldrCommReset(void) CYSMALL 
{
    ...
    I2C_MPU9250_Master_slRdBufSize = 0u;
...
/*******************************************************************************
* Function Name: I2C_MPU9250_Master_CyBtldrCommWrite
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
cystatus I2C_MPU9250_Master_CyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut) CYSMALL
         
{
    ...
    if((NULL != pData) && (size > 0u))
    {
        ...
        (void) memcpy((void *) I2C_MPU9250_Master_slReadBuf, (void *) pData, size);
        *count = size;  /* Buffer was copied to I2C buffer */

        /* Buffer is free to be read */
        I2C_MPU9250_Master_slRdBufSize = ((uint8) size);

        while(0u != timeoutMs)  /* Wait till response is read */
        {
            /* Check if host complete read */
            if(I2C_MPU9250_Master_slRdBufIndex == ((uint8) size))
            {
                I2C_MPU9250_Master_slRdBufSize  = 0u;
                I2C_MPU9250_Master_slRdBufIndex = 0u;
            ...
            CyDelay(I2C_MPU9250_Master_WAIT_1_MS);
            timeoutMs--;
        }
    }
...
/*******************************************************************************
* Function Name: I2C_MPU9250_Master_CyBtldrCommRead
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2C_MPU9250_Master_slWriteBuf - The global variable used to store a
*                                received command.
*
*******************************************************************************/
         ...
         
{
    ...
    if((NULL != pData) && (size > 0u))
    {
        ...
        while(0u != timeoutMs)  /* Wait for command from host */
        {
            ...
            if(0u != (I2C_MPU9250_Master_slStatus & I2C_MPU9250_Master_SSTAT_WR_CMPLT))
            {
                /* Define how many bytes host has been written */
                byteCount = I2C_MPU9250_Master_slWrBufIndex;
                *count = (uint16) byteCount;

                /* Copy command to pData buffer */
                (void) memcpy((void *) pData, (void *) I2C_MPU9250_Master_slWriteBuf,
                              I2C_MPU9250_Master_MIN_UNT16(byteCount, size));
