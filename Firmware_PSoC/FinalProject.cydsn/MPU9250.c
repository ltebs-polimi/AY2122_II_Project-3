...
#ifndef MPU9250_FIFO_MODE_MASK 
    #define MPU9250_FIFO_MODE_MASK 0x43 // -> 01000011 bits to set (|)
#endif

#ifndef MPU9250_COUNTH_BITS_FIFO_MASK
    #define MPU9250_COUNTH_BITS_FIFO_MASK 0xE0  // -> 11100000 bits to clear (&onda)
#endif
...
MPU9250_Acc_FS MPU9250_GetAccFS(void) {
    ...
    temp &= MPU9250_ACC_FS_MASK; //&= significa che 00011000
    // Shift them by 3
    temp = temp >> 3;
    return temp;
}
...
void MPU9250_SetGyroFS(MPU9250_Gyro_FS fs) {
    ...
    uint8_t temp = MPU9250_I2C_Read(MPU9250_I2C_ADDRESS, MPU9250_GYRO_CONFIG_REG);
    // Then, we clear bits [4:3]
    temp &= ~MPU9250_GYRO_FS_MASK; //&onda significa che 11100111
    // Lastly, we write the new byte to the register
...
void MPU9250_ConfigFifoMode(void) {
    // Set FIFO MODE bit [6] in MPU9250_CONFIG_REG
    	MPU9250_I2C_Write(MPU9250_I2C_ADDRESS, MPU9250_CONFIG_REG,  
        ( MPU9250_I2C_Read(MPU9250_I2C_ADDRESS, MPU9250_CONFIG_REG) | MPU9250_FIFO_MODE_MASK ));            
}
